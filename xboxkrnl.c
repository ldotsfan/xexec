...
#define XBOXKRNL_MAX_CPUS    1          /* non-SMP! */
...
#define INT3 \
        do { \
            PRINTF( \
                XEXEC_DBG_ALL, \
                "INT3:\n" \
                "breakpoint triggered at line %i of file: '%s'\n" \
                "last errno: '%s' (%i)\n" \
...
#define MEM_POP(i,a)                    xboxkrnl_mem_pop(i,(void *)(a),0)
#define MEM_POP__LOCKED(i,a)            xboxkrnl_mem_pop(i,(void *)(a),1)
...
#define MEM_DIRTY_PARENT_RET__LOCKED(a) xboxkrnl_mem_dirty_parent_ret__locked((void *)(a))
#define MEM_DIRTY_CREATE(a,s)           xboxkrnl_mem_dirty_create((void *)(a),s,0)
#define MEM_DIRTY_CREATE__LOCKED(a,s)   xboxkrnl_mem_dirty_create((void *)(a),s,1)
#define MEM_DIRTY_DESTROY(a)            xboxkrnl_mem_dirty_destroy((void *)(a))
#define MEM_DIRTY_SET(a)                xboxkrnl_mem_dirty_set((void *)(a))
#define MEM_DIRTY_SET__LOCKED(a)        xboxkrnl_mem_dirty_set__locked((void *)(a),NULL)
...
#define MEM_ALLOC_RESERVE__LOCKED(a,s)  xboxkrnl_mem_alloc(MEM_RESERVE,a,s,MEM_PROT_NONE,MAP_PRIVATE|MAP_ANONYMOUS,-1,0,1)
#define MEM_ALLOC_EXEC(a,s)             xboxkrnl_mem_alloc(MEM_EXEC,a,s,MEM_PROT_EXEC,MAP_SHARED|MAP_ANONYMOUS,-1,0,0)
#define MEM_ALLOC_EXEC__LOCKED(a,s)     xboxkrnl_mem_alloc(MEM_EXEC,a,s,MEM_PROT_EXEC,MAP_SHARED|MAP_ANONYMOUS,-1,0,1)
#define MEM_ALLOC_MAP(i,a,s)            xboxkrnl_mem_alloc(i,a,s,MEM_PROT_RDWR,MAP_SHARED|MAP_ANONYMOUS,-1,0,0)
#define MEM_ALLOC_MAP__LOCKED(i,a,s)    xboxkrnl_mem_alloc(i,a,s,MEM_PROT_RDWR,MAP_SHARED|MAP_ANONYMOUS,-1,0,1)
#define MEM_REALLOC_HEAP(a,s)           xboxkrnl_mem_realloc(MEM_HEAP,a,s,0)
...
xboxkrnl_mem_dirty_set__locked(void *addr, xboxkrnl_mem *m) {
    register xboxkrnl_mem_table *t = &xboxkrnl_mem_tables[MEM_DIRTY];
...
void *
xboxkrnl_mem_alloc(int index, void *addr, size_t size, int prot, int flags, int fd, off_t offset, int locked) {
    register xboxkrnl_mem_table *t = &xboxkrnl_mem_tables[index];
    register xboxkrnl_mem *m;
...
void *
xboxkrnl_mem_realloc(int index, void *ptr, size_t size, int locked) {
    register xboxkrnl_mem_table *t = &xboxkrnl_mem_tables[index];
    register xboxkrnl_mem *m;
    register xboxkrnl_mem *n;
    register ssize_t i;
    ...
    switch (index) {
    case MEM_HEAP:
        if (!ptr) return MEM_ALLOC_HEAP(size);
        break;
    case MEM_MAP:
    case MEM_ALLOC:
        if (!ptr) return MEM_ALLOC_MAP(index, NULL, size);
        break;
    default:
        INT3;
        break;
    ...
    /* find the page */
    for (ret = NULL, m = NULL, i = 0; i <= t->sz; ++i) {
        if (i >= t->sz) INT3;
        m = &t->mem[i];
        n = (i + 1 < t->sz) ? &t->mem[i + 1] : NULL;
        if (m->addr == ptr) {
            ...
            case MEM_HEAP:
                ...
                break;
            case MEM_MAP:
                if (ret + size > MEM_HEAP_BASE) ret = NULL;
                break;
            }
            break;
        }
...
/* timestamp counter hook (i686-only host) */
static pthread_mutex_t      xboxkrnl_tsc_mutex    = PTHREAD_MUTEX_INITIALIZER;
static int                  xboxkrnl_tsc_hook     = 0;
...
#if defined(__i386__) /* i686-only host */
    return (xboxkrnl_tsc_hook) ? prctl(PR_SET_TSC, PR_TSC_ENABLE) : -1;
#else
    errno = ENOTSUP;
...
#if defined(__i386__) /* i686-only host */
    return (xboxkrnl_tsc_hook) ? prctl(PR_SET_TSC, PR_TSC_SIGSEGV) : -1;
#else
    errno = ENOTSUP;
...
xboxkrnl_tsc_init(void) {
    ...
    do {
        ...
        if (xboxkrnl_tsc_off() < 0) {
            PRINTF(XEXEC_DBG_ERROR, "warning: failed to hook TSC register: prctl(): '%s'", strerror(errno));
            break;
        }
        xboxkrnl_tsc_on();
...
void *
xboxkrnl_entry_pthread(void *arg) {
...
static ssize_t                          xboxkrnl_thread_list_sz                    = 0;
...
#define THREAD_PUSH(r,a)                xboxkrnl_thread_push(r,(void *)(a),NULL,#r"()",0)
#define THREAD_PUSH__LOCKED(r,a)        xboxkrnl_thread_push(r,(void *)(a),NULL,#r"()",1)
#define THREAD_PUSH2(r,a,c)             xboxkrnl_thread_push(r,(void *)(a),(void *)(c),#r"()",0)
#define THREAD_PUSH2__LOCKED(r,a,c)     xboxkrnl_thread_push(r,(void *)(a),(void *)(c),#r"()",1)
#define THREAD_PUSH3(r,a,c,n)           xboxkrnl_thread_push(r,(void *)(a),(void *)(c),n,0)
#define THREAD_PUSH3__LOCKED(r,a,c,n)   xboxkrnl_thread_push(r,(void *)(a),(void *)(c),n,1)
...
#define THREAD_JOIN(t)                  xboxkrnl_thread_join(t,NULL)
#define THREAD_JOIN2(t,ret)             xboxkrnl_thread_join(t,ret)
#define THREAD_WAIT(t)                  xboxkrnl_thread_wait(t)
#define THREAD_POST(t)                  xboxkrnl_thread_post(t)

int
xboxkrnl_thread_join(xboxkrnl_thread *t, void **retval) {
    return pthread_join(t->id, retval);
}
...
xboxkrnl_thread_push(void *(*routine)(void *), void *arg, void *context, const char *name, int locked) {
    ...
    t->sp   = sp;
    t->spsz = STACKSIZE;
    if (pthread_attr_setstack(&t->attr, t->sp, t->spsz)) INT3;
    ...
    i = xboxkrnl_strlen(name);
    i = (i >= 15) ? i - 15 : 0;
    if (pthread_setname_np(t->id, &t->name[i])) INT3;
...
#define THREAD(x,y) \
        t = THREAD_PUSH3(xboxkrnl_worker, x, x, #y); \
    ...
    if (!phys) {
        do {
            if ((ret = nv2a->op->read(addr, val, sz))) {
                //TODO: trampoline hook NV_PFB_WBC spinlock read
                break;
            }
...
xboxkrnl_NtFreeVirtualMemory( /* 199 (0x0c7) */
    ...
    if (FreeType & XBOXKRNL_MEM_RELEASE /* 0x8000 */) {
        ret = MEM_FREE_MAP(MEM_MAP, *BaseAddress);
...
xboxkrnl_RtlEqualString( /* 279 (0x117) */
    ...
    len1 = xboxkrnl_strlen(String1->Buffer) + 1;
    len2 = xboxkrnl_strlen(String2->Buffer) + 1;
...
xboxkrnl_RtlInitAnsiString( /* 289 (0x121) */
    ...
    DestinationString->Length        = xboxkrnl_strlen(SourceString);
